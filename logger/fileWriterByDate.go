...
func setupLoggerCommonByDate(log *logrus.Logger, level logrus.Level, fileName, logDir string, rotateMaxAge, skip int, report bool) {
	if log == nil {
...
func (p *LogFileWriterByDate) millRunOnce() error {
	...
	for _, file := range oldFiles {
		if err := os.Remove(file); err != nil {
			logx.Errorf("remove old log file: %s err: %s\n", file, err.Error())
		}
	}

	logx.Debugf("success do millRunOnce, files: %+v, dirPath: %s\n", oldFiles, p.dirPath)
	return nil
}
